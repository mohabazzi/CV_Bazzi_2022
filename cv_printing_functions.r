...
# Function has been modifies to enable hyperlink printing. Modified version cant handle special charcters in the google spreadsheet titles. This should be relatively easy to fix.
sanitize_links <- function(cv, text){
  if(cv$pdf_mode){
    # Extract link titles and destinations using a regex pattern
    link_pattern <- "\\[(.*?)\\]\\((.*?)\\)"
    link_titles <- stringr::str_extract_all(text, link_pattern)[[1]]
    
    if (length(link_titles) > 0) {
      # Iterate over each link match and format it for PDF rendering
      for (link in link_titles) {
        # Extract the link title and destination
        link_parts <- stringr::str_match(link, link_pattern)
        link_title <- link_parts[2]
        link_destination <- link_parts[3]
        
        # Format the link for PDF rendering
        pdf_link <- paste("<a href='", link_destination, "'>", link_title, "</a>")
        text <- stringr::str_replace(text, link, pdf_link)
      }
    }
  }
...
#' @description Construct a bar chart of skills: "#969696"  "#d9d9d9"
#' @param out_of The relative maximum for skills. Used to set what a fully filled in skill bar is.
print_skill_bars <- function(cv, out_of = 5, bar_color = "#2D716F", bar_background = "#544948", glue_template = "default"){
